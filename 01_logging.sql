/* ============================================================================
   01_logging.sql
   Purpose: Cross-cutting RDAP metadata for logging/annotation.
   Scope: No foreign keys; safe to deploy independently.

   Tables:
     - events   : lifecycle and operational events for any RDAP object
     - links    : RFC-compliant link relations attached to objects
     - remarks  : free-form annotations on objects
     - notices  : registry/operator notices

   Run order: 2 of 4  (bootstrap → logging → TLD → domain)
   Depends on: 00_bootstrap_extensions.sql
   Referenced by: None directly; read by services across layers
   Safe to re-run: yes (IF NOT EXISTS guards recommended)
   Notes:
     - Consider adding partitioning/indexing later if event volume grows.
============================================================================ */
