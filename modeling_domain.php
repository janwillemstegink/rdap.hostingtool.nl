...
function SwitchTranslation(translation)	{
	...
	if (translation == 0)	{		
		...
		document.getElementById("event_last_validated_registrant").textContent = proposed;
		...
		document.getElementById("backup_web_id").textContent = proposed;
		document.getElementById("backup_protected").textContent = derived;
		...
		document.getElementById("view_role").textContent = "De indexering op web-ID is gewenst. Het land van de daadwerkelijke gebruiker maakt frequente controles mogelijk.";
		...
		document.getElementById("event_last_validated_registrant").textContent = proposed + "Entiteitsvalidatie vanuit een nationaal domeincontroleregister; en t.b.v. zoekmachines.";
		document.getElementById("event_last_validated_reseller").textContent = proposed;
		document.getElementById("event_last_validated_registrar").textContent = proposed;
		...
		document.getElementById("billing_protected").textContent = derived;
		document.getElementById("backup_role").textContent = proposed + "Een back-up contact heeft de noodzakelijke toegang bij vervanging.";
		document.getElementById("backup_web_id").textContent = proposed;
		document.getElementById("backup_protected").textContent = derived;
		...
		document.getElementById("view_role").textContent = "The indexing by web ID is desired. The country of the actual user facilitates frequent checks.";	
		...
		document.getElementById("event_last_validated_registrant").textContent = proposed + "Entity validation from a national domain control registry; and for search engines.";
		document.getElementById("event_last_validated_reseller").textContent = proposed;
		document.getElementById("event_last_validated_registrar").textContent = proposed;
		...
		document.getElementById("billing_protected").textContent = derived;
		document.getElementById("backup_role").textContent = proposed + "A backup contact has the necessary access in the event of replacement.";
		document.getElementById("backup_web_id").textContent = proposed;
		document.getElementById("backup_protected").textContent = derived;		
		...
		document.getElementById("view_role").textContent = "Die Indizierung per Web-ID ist erwünscht. Das Land des tatsächlichen Benutzers erleichtert häufige Überprüfungen.";
		...
		document.getElementById("event_last_validated_registrant").textContent = proposed + "Entitätsvalidierung durch ein nationales Domänenkontrollregister; und für Suchmaschinen.";
		document.getElementById("event_last_validated_reseller").textContent = proposed;
		document.getElementById("event_last_validated_registrar").textContent = proposed;
		...
		document.getElementById("billing_protected").textContent = derived;
		document.getElementById("backup_role").textContent = proposed + "Im Ersatzfall verfügt ein Backup-Kontakt über den nötigen Zugriff.";
		document.getElementById("backup_web_id").textContent = proposed;
		document.getElementById("backup_protected").textContent = derived;
		...
		document.getElementById("view_role").textContent = "L'indexation par web ID est souhaitée. Le pays de l'utilisateur réel facilite des contrôles fréquents.";
		...
		document.getElementById("event_last_validated_registrant").textContent = proposed + "Validation d'entité à partir d'un registre national de contrôle de domaine; et pour les moteurs de recherche.";
		document.getElementById("event_last_validated_reseller").textContent = proposed;
		document.getElementById("event_last_validated_registrar").textContent = proposed;
		...
		document.getElementById("billing_protected").textContent = derived;
		document.getElementById("backup_role").textContent = proposed + "Un contact de secours dispose des accès nécessaires en cas de remplacement.";
		document.getElementById("backup_web_id").textContent = proposed;
		document.getElementById("backup_protected").textContent = derived;		
...
foreach ($xml1->xpath('//domain') as $item)	{
	...
	$html_text .= '<tr><td><button style="cursor:pointer;font-size:1.05rem" onclick="SwitchDisplay(31)">events +/-</button></td><td></td><td></td></tr>';
	...
	$html_text .= '<tr id="312" style="display:none"><td>event_last_uploaded</td><td>'.$item->event_last_uploaded.'</td><td id="event_last_uploaded"></td></tr>';
	$html_text .= '<tr id="313" style="display:none"><td>event_expiration_grace_until</td><td>'.$item->event_expiration_grace_until.'</td><td></td></tr>';
	$html_text .= '<tr><td>event_last_changed</td><td>'.$item->event_last_changed.'</td><td></td></tr>';
	$html_text .= '<tr id="314" style="display:none"><td>event_last_transferred</td><td>'.$item->event_last_transferred.'</td><td></td></tr>';
	$html_text .= '<tr id="315" style="display:none"><td>event_last_validated_registrant</td><td>'.$item->event_last_validated_registrant.'</td><td id="event_last_validated_registrant"></td></tr>';
	$html_text .= '<tr id="316" style="display:none"><td>event_last_validated_reseller</td><td>'.$item->event_last_validated_reseller.'</td><td id="event_last_validated_reseller"></td></tr>';
	$html_text .= '<tr id="317" style="display:none"><td>event_last_validated_registrar</td><td>'.$item->event_last_validated_registrar.'</td><td id="event_last_validated_registrar"></td></tr>';
	$html_text .= '<tr><td><hr></td><td><hr></td><td><hr></td></tr>';
	...
	$html_text .= '<tr><td><button style="cursor:pointer;font-size:1.05rem" onclick="SwitchDisplay(42)">technical +/-</button></td><td></td><td id="tech_role"></td></tr>';
