...
function SwitchTranslation(translation)	{
	...
	if (translation == 0)	{
		...
		document.getElementById("zone_language_codes").textContent = modified;
		...
		document.getElementById("resource_upload_at").textContent = "";
		...
		document.getElementById("zone_backend_operator_presented_name").textContent = proposed;
		document.getElementById("zone_language_codes").textContent = modified + "Geeft ondersteunde talen voor de zone aan.";
		...
		document.getElementById("resource_upload_at").textContent = "Datum en tijdstip van de RDAP-database-update in Zoeloe-tijd (Coordinated Universal Time - UTC).";
		...
		document.getElementById("domain_statuses").textContent = "De waarde 'redemption period' is info over herstel. De waarde 'pending delete' is van toepassing in de laatste fase.";
		...
		document.getElementById("zone_backend_operator_presented_name").textContent = proposed;
		document.getElementById("zone_language_codes").textContent = modified + "Indicates supported languages for the zone.";
		...
		document.getElementById("resource_upload_at").textContent = "Date and time of RDAP database update in Zulu time (Coordinated Universal Time - UTC).";
		...
		document.getElementById("domain_statuses").textContent = "The 'redemption period' value is info about recovery. The 'pending delete' value applies in the final phase.";
		...
		document.getElementById("domain_extensions").textContent = "'Eligibility': How a domain fulfills a specific requirement in a top-level domain zone.";
		...
		document.getElementById("zone_backend_operator_presented_name").textContent = proposed;
		document.getElementById("zone_language_codes").textContent = modified + "Gibt die unterstützten Sprachen der Zone an.";
		...
		document.getElementById("resource_upload_at").textContent = "Datum und Uhrzeit der RDAP-Datenbankaktualisierung in Zulu-Zeit (Koordinierte Weltzeit – UTC).";
		...
		document.getElementById("domain_statuses").textContent = "Der Wert 'redemption period' ist Info zur Wiederherstellung. Der Wert 'pending delete' gilt in der Endphase.";
		...
		document.getElementById("zone_backend_operator_presented_name").textContent = proposed;
		document.getElementById("zone_language_codes").textContent = modified + "Indique les langues prises en charge pour la zone.";
		...
		document.getElementById("resource_upload_at").textContent = "Date et heure de mise à jour de la base de données RDAP en heure Zulu (Temps Universel Coordonné - UTC).";
		...
		document.getElementById("domain_statuses").textContent = "La valeur 'redemption period' est infos de récupération. La valeur 'pending delete' s'applique dans la phase finale.";
...
if (true or $pd == mb_strtolower($data[$pd]['domain']['ascii_name']) or empty($data[$pd]['domain']['ascii_name']))	{
	...
	$html_text .= '<tr id="291" style="display:none"><td>resource resource_upload_at</td><td>'.$data[$pd]['resource']['resource_upload_at'].'</td><td id="resource_upload_at"></td></tr>';
	...
	$html_text .= '<tr id="292" style="display:none"><td>resource registry_source</td><td>'.((strlen($data[$pd]['resource']['registry_source'])) ? '<a href='.$data[$pd]['resource']['registry_source'].' target="_blank">Registry Response</a> - <a href="' . htmlspecialchars($validation_registry, ENT_QUOTES, "UTF-8") . '" target="_blank">validator.rdap.org</a>' : 'none').'</td><td id="background_registry_source"></td></tr>';
	...
	$html_text .= '<tr id="293" style="display:none"><td>resource registrar_source eg. <a style="font-size: 0.9rem" href="https://rdap.cscglobal.com/dbs/rdap-api/v1/domain/icann.com" target="_blank">icann.com</a> <a style="font-size: 0.9rem" href="https://rdap.metaregistrar.com/domain/fryslan.frl" target="_blank">fryslan.frl</a></td><td>'.((strlen($data[$pd]['resource']['registrar_source'])) ? '<a href='.$data[$pd]['resource']['registrar_source'].' target="_blank">Registrar Response</a> - <a href="' . htmlspecialchars($validation_registrar, ENT_QUOTES, "UTF-8") . '" target="_blank">validator.rdap.org</a>' : 'none').'</td><td id="resource_registrar_source"></td></tr>';
	$html_text .= '<tr id="294" style="display:none"><td>resource registrar_public_ids</td><td>'.((strlen($data[$pd]['resource']['registrar_public_ids'])) ? $data[$pd]['resource']['registrar_public_ids'] : 'none').'</td><td id="resource_registrar_public_ids"></td></tr>';
	$html_text .= '<tr id="295" style="display:none"><td>resource registrar_complaint_url</td><td>'.((strlen($data[$pd]['resource']['registrar_complaint_url'])) ? '<a href='.$data[$pd]['resource']['registrar_complaint_url'].' target="_blank">icann.org/wicf</a>' : 'none').'</td><td id="resource_registrar_complaint_url"></td></tr>';
	$html_text .= '<tr id="296" style="display:none"><td>resource status_explanation_url</td><td>'.((strlen($data[$pd]['resource']['status_explanation_url'])) ? '<a href='.$data[$pd]['resource']['status_explanation_url'].' target="_blank">icann.org/epp</a>' : 'none').'</td><td id="resource_status_explanation_url"></td></tr>';
	...
	$html_text .= '<tr id="309" style="display:none;vertical-align:top"><td>domain extensions</td><td>'.$data[$pd]['domain']['extensions'].'</td><td id="domain_extensions"></td></tr>';
	$html_text .= '<tr id="3010" style="display:none;vertical-align:top"><td>domain remarks</td><td>'.$data[$pd]['domain']['remarks'].'</td><td></td></tr>';
