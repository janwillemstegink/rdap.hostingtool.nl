...
function SwitchTranslation(translation)	{
	...
	if (translation == 0)	{		
		...
		document.getElementById("view_role").textContent = "Er is een plan voor centrale RDAP-servers. Dit kan werken voor Web ID-indexering.";
		...
		document.getElementById("view_role").textContent = "There is a plan for central RDAP servers. This can work for Web ID indexing.";	
		...
		document.getElementById("view_role").textContent = "Es gibt einen Plan für zentrale RDAP-Server. Dies kann für die Web-ID-Indizierung funktionieren.";
		...
		document.getElementById("view_role").textContent = "Il existe un plan pour les serveurs RDAP centraux. Cela peut fonctionner pour l’indexation des Web ID.";
...
foreach ($xml1->xpath('//domain') as $item)	{
	...
	$html_text .= '<tr id="306" style="display:none"><td>domain_last_cycle_control</td><td>'.$item->domain_last_cycle_control.'</td><td id="domain_last_cycle_control"></td></tr>';
	$html_text .= '<tr><td><hr></td><td><hr></td><td><hr></td></tr>';
