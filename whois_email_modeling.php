...
$dataFile = 'whois_email_data.xml';
$inputdomain = 'webhostingtech.nl';
$url1a = "https://whois.hostingtool.nl/whois_compose_data/index.php?domain=$inputdomain&format=xml&type=1";
...
foreach ($xml1->xpath('//domain') as $item)	{
	...
	$html_text .= '<tr><td>registrant_state</td><td>'.$item->registrant->registrant_state.'</td></tr>';	
	...
	$html_text .= '<tr><td>admin_state</td><td>'.$item->admin->admin_state.'</td></tr>';
	...
	$html_text .= '<tr><td COLSPAN="2"><br />Facturering (als zaak van de registry in discussie) / Billing (as a registry matter under discussion):</td></tr>';
	...
	$html_text .= '<tr><td>billing_state</td><td>'.$item->billing->billing_state.'</td></tr>';
