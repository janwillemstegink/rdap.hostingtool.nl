...
foreach ($xml1->xpath('//domain') as $item)	{
	...
	if (!empty($item->reseller->reseller_name))	{
		if (strlen(trim($item->reseller->reseller_name)))	{
			...
			<td>De verwerkersovereenkomst is mogelijk tussen de reseller en de houder.</td>
			<td>The processing agreement is possible between the reseller and the holder.</td></tr>';
