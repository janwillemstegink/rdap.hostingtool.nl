...
foreach ($xml->xpath('//domain') as $item)	{
	...
	<td>Een web_id bij de KVK kan goed werken voor identificatie (gewenst sinds 2006).</td><td>A web_id at the Chamber of Commerce can work well for identification.</td></tr>';
	$html_text .= '<tr><td><b>registrant_holder_business_use</b></td><td><b>'.$item->registrant->registrant_holder_business_use.'</b></td>
	<td>business_use: Zakelijke transparantie versus privacy van natuurlijke personen.</td><td>business_use: Business transparency versus natural person privacy.</td></tr>';
...
simplexml_load_string($item->asXML());	
	$html_text .= '<tr><td></td><td></td><td COLSPAN="2">'.$item->restrictions_legally.'</td></tr>';
	$html_text .= '<tr><td></td><td></td><td COLSPAN="2">'.$item->restrictions_translated.'</td></tr>';
	$html_text .= '<tr><td><hr></td><td><hr></td><td><hr></td><td><hr></td></tr>';
	break;
}
