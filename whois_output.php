...
if (file_exists($filesPath.$inputFile) and false)	{
	$xml = simplexml_load_file($filesPath.$inputFile) or die("Cannot load xml file 1 from path,");
}
elseif (file_exists($inputFile))	{
	$xml = simplexml_load_file($inputFile) or die("Cannot load xml file 1 from same folder.");
}
else	{
	//$url = "http://whois.hostingtool.nl/whois_fetch.php?domain=".$inputdomain."&format=xml&type=1";
	$url = "http://whois.hostingtool.nl/whois_input.xml";
...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><meta name="robots" content="noindex"><title>Whois modeling for Trade Register use</title>';
?><script>
...
$html_text .= '<tr><td><b>Web Domain Whois Modeling</b></td><td COLSPAN=3>Whois lookup based on a file in XML format written by the registry - https://github.com/janwillemstegink/xml-whois</td></tr>';
foreach ($xml->xpath('//domain') as $item)	{
	...
	<td>Voor zakelijk gebruik is de houdernaam zichtbaar. Directe verantwoordelijkheid kan gebaseerd zijn op mogelijke toegang van de houder en anders de reseller.</td>
	<td>For business use, the holder name is visible. Direct responsibility may be based on possible access of the holder and otherwise the reseller.</td></tr>';
	$html_text .= '<tr><td><b>registrant_holder_web_publish</b></td><td><b>'.$item->registrant->registrant_holder_web_publish.'</b></td>
	<td>Met web_publish op ja, kunnen zoekmachines legaal publiceren.</td><td>With web_publish set to yes, search engines can publish legally.</td></tr>';
	$html_text .= '<tr><td><b>registrant_holder_web_id</b></td><td><b>'.$item->registrant->registrant_holder_web_id.'</b></td>
	<td>Een web_id in het Handelsregister kan goed werken voor identificatie (verzoek is uit 2006). De KvK kan een Whois-lijst efficiënt automatiseren.</td>
	<td>A web_id in the Trade Register can work well for identification (request is from 2006). The Chamber of Commerce can efficiently automate a Whois list.</td></tr>';
	$html_text .= '<tr><td><b>registrant_holder_business_use</b></td><td><b>'.$item->registrant->registrant_holder_business_use.'</b></td>
	<td>De business_use is voor transparantie, en de privacy voor natuurlijke personen.</td><td>The business_use is for transparency, and the privacy for natural persons.</td></tr>';
